...
#define GPIO_HSYNC 28
#define GPIO_VSYNC 29
#define GPIO_RGB_BASE 16
#define RGB_PINCOUNT 12

#include "vidout.pio.h"
#include "vgaout.h"

#define VGA_PIO pio0
#define VGA_SPINLOCK_ID 31

#define PIXEL_FREQ 25.2/*MHz*/
#define PIXELS_PER_LINE 804
#define LINES_PER_FRAME 523
#define LINES_IN_BACK_PORCH 31
...
static void vga_hsync_setup(PIO pio, uint sm) {
    ...
    sm_config_set_out_pins(&c, GPIO_HSYNC, 1);
    sm_config_set_set_pins(&c, GPIO_HSYNC, 1);

    // Configure the pins as outputs & connect to the PIO
    pio_sm_set_consecutive_pindirs(pio, sm, GPIO_HSYNC, 1, true);
...
static void vga_vsync_setup(PIO pio, uint sm) {
    ...
    // Map the state machine's OUT pin group to the sync signal pin
    sm_config_set_out_pins(&c, GPIO_VSYNC, 1);
    sm_config_set_set_pins(&c, GPIO_VSYNC, 1);

    // Configure the pins as outputs & connect to the PIO
    pio_sm_set_consecutive_pindirs(pio, sm, GPIO_VSYNC, 1, true);
...
static void vga_data_setup(PIO pio, uint sm) {
    ...
    sm_config_set_out_pins(&c, GPIO_RGB_BASE, RGB_PINCOUNT);
    sm_config_set_set_pins(&c, GPIO_RGB_BASE, RGB_PINCOUNT);
    ...
    // Configure the pins as outputs & connect to the PIO
    pio_sm_set_consecutive_pindirs(pio, sm, GPIO_RGB_BASE, RGB_PINCOUNT, true);
    for(int i=0; i < RGB_PINCOUNT; i++) {
        pio_gpio_init(pio, GPIO_RGB_BASE+i);
    }
...
static void __noinline __time_critical_func(trigger_ready_scanline_dma)() {
...
static void __noinline __time_critical_func(vga_dma_irq_handler)() {
    spin_lock_t *lock = spin_lock_instance(VGA_SPINLOCK_ID);
    struct vga_scanline *active_scanline = &scanline_queue[scanline_queue_tail];
...
void vga_init() {
    if(!vga_initialized) {
        spin_lock_claim(VGA_SPINLOCK_ID);
        spin_lock_init(VGA_SPINLOCK_ID);

        // Setup the PIO state machines
        vga_hsync_setup(VGA_PIO, VGA_HSYNC_SM);
        vga_vsync_setup(VGA_PIO, VGA_VSYNC_SM);
        vga_data_setup(VGA_PIO, VGA_DATA_SM);

        // Setup the DMA channel for writing to the data PIO state machine
        vga_dma_channel = 8; //dma_claim_unused_channel(true);
        dma_channel_claim(vga_dma_channel);
        dma_channel_config c = dma_channel_get_default_config(vga_dma_channel);
        channel_config_set_transfer_data_size(&c, DMA_SIZE_32);
        channel_config_set_dreq(&c, pio_get_dreq(VGA_PIO, VGA_DATA_SM, true));
        dma_channel_configure(vga_dma_channel, &c, &VGA_PIO->txf[VGA_DATA_SM], NULL, 0, false);
    ...
    pio_enable_sm_mask_in_sync(VGA_PIO, (1 << VGA_HSYNC_SM) | (1 << VGA_VSYNC_SM) | (1 << VGA_DATA_SM));
}

void vga_stop() {
    pio_set_sm_mask_enabled(VGA_PIO, (1 << VGA_HSYNC_SM) | (1 << VGA_VSYNC_SM) | (1 << VGA_DATA_SM), false);
}

// Set up for a new display frame
void __noinline __time_critical_func(vga_prepare_frame)() {
...
struct vga_scanline * __noinline __time_critical_func(vga_prepare_scanline)() {
...
void __noinline __time_critical_func(vga_submit_scanline)(struct vga_scanline *scanline) {
    spin_lock_t *lock = spin_lock_instance(VGA_SPINLOCK_ID);
